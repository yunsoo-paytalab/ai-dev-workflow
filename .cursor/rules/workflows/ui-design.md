...
1. 디자인 시스템 적용 (사용자 확인 후 기존 시스템 우선 활용)
   - **⚠️ 중요**: 기존에 적용된 스타일을 절대 변경하지 않도록 각별하게 주의
   - 기존 스타일과의 호환성 유지 우선
   - 기존 스타일이 있는 경우 해당 스타일을 우선적으로 보존
2. 컴포넌트 최적화
   - 분리가 가능한 컴포넌트가 있는지 확인
   - 중복된 코드가 있는지 확인
   - 컴포넌트 재사용성 개선
   - Props 인터페이스 최적화
   - 불필요한 리렌더링 방지
   - **⚠️ 중요**: 스타일 변경 없이 로직만 최적화
3. 접근성 개선
   - **⚠️ 중요**: 기존 스타일을 유지하면서 접근성 속성만 추가/개선
4. 성능 최적화
   - **⚠️ 중요**: 기존 스타일을 보존하면서 성능만 개선
...
- [ ] 기존 스타일 보존 원칙 확인 및 적용
- [ ] 디자인 시스템이 적용됨 (사용자 확인 후 기존 시스템 우선 활용)
- [ ] 컴포넌트가 최적화됨
  - [ ] 분리가 가능한 컴포넌트 확인 및 분리
  - [ ] 중복된 코드 확인 및 제거
  - [ ] 컴포넌트 재사용성 개선
  - [ ] Props 인터페이스 최적화
  - [ ] 불필요한 리렌더링 방지
  - [ ] **기존 스타일 변경 없이 로직만 최적화됨**
- [ ] 접근성이 개선됨
  - [ ] **기존 스타일 유지하면서 접근성 속성만 추가/개선됨**
- [ ] 성능이 최적화됨
  - [ ] **기존 스타일 보존하면서 성능만 개선됨**
- [ ] 코드 품질이 향상됨
- [ ] 재사용성이 개선됨
- [ ] 일관성이 확보됨
...
#### ⚠️ 중요: 기존 스타일 변경 금지 원칙

- **절대 변경 금지**: 기존에 적용된 CSS, 스타일 속성, 클래스명을 절대 변경하지 않음
- **호환성 우선**: 기존 스타일과의 호환성을 최우선으로 고려
- **추가만 허용**: 기존 스타일에 영향을 주지 않는 추가 작업만 수행

#### 작업 시 주의사항

1. **스타일 분석 우선**: 작업 전 기존 스타일을 철저히 분석
2. **변경 영향도 검토**: 모든 변경사항이 기존 스타일에 미치는 영향 검토
3. **점진적 적용**: 기존 스타일을 보존하면서 필요한 기능만 추가
4. **검증 필수**: 변경 후 기존 스타일이 그대로 유지되는지 확인
