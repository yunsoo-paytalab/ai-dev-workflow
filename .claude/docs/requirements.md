...
| 항목                        | 입력 형식                                                                                                                                                  |
| --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **ID**                      | - 형식: `p#{1자리 영문 + 3자리 숫자}` (예: pa123)<br>- 랜덤 기본값 제공, 수정 가능 (중복 불가)<br>- 랜덤 생성 버튼 제공                                    |
| **회원 아이디 (Member ID)** | - 회원 계정 생성 시만 표시<br>- 형식: `mi#{3자리 숫자}` (예: mi969)<br>- 랜덤 기본값 제공, 수정 가능<br>- 랜덤 생성 버튼 제공                              |
...
**목적**: 재무 로직과 분리된 상품권/패스머니 안전한 지급 및 회수

##### 1. 상품권

- **지급**: 백오피스 활용
- **회수**:
  1. 유저 식별자 또는 번호로 테스트 대상 검색
  2. 보유 상품권 목록 중 선택하여 일괄 삭제
  3. 검증: 상품권 사용된 주문이 모두 취소되었는지 확인
  4. 미취소 주문 존재 시 삭제 불가
...
- **지급 플로우**:
  1. 유저 식별자 또는 번호로 테스트 대상 검색
  2. 충전수단 선택 (계좌이체/카드결제) - 기본값 제공 및 수정 가능
  3. 패스머니 금액 입력 (100 ~ 999,900)

- **회수 플로우**:
  1. 유저 식별자 또는 번호로 테스트 대상 검색
  2. 보유 패스머니 금액 중 회수할 금액 입력
     - 입력 금액 ≤ 보유 패스머니
  3. 검증: 패스머니 사용된 주문이 모두 취소되었는지 확인
  4. 미취소 주문 존재 시 회수 불가
