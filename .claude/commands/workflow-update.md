...
Grep/Glob 도구를 사용하여 영향 범위를 직접 분석합니다:

1. **Feature ID로 문서 검색**: `Grep`으로 Feature ID가 언급된 모든 문서 탐색
2. **의존성 분석**: feature-list.md에서 의존 관계 확인
3. **영향 분류**:
   - 직접 영향받는 문서 (Feature 상세, research, plan)
   - 간접 영향받는 문서 (의존성 있는 다른 Feature)
   - 구현 파일 (수동 작업 필요 표시)
...
document-updater 에이전트를 사용하여 문서를 업데이트합니다:
